...
class Simbol:
    def __init__(self, color, posX, posY, text, size,display):
        ...
        self.text = text 
        self.rect = font.render(self.text, True, self.color)
        self.canISpin = False
...
def creatWheel(num, x):
    ...
    randObjArray = random.sample(objArray, len(objArray))
    
    for i, v in enumerate(randObjArray):
        randObjArray[i].pos['y'] = newY[i]
...
def drawWindow(win, wheels):
    ...
    for objs in wheels:
        for obj in objs:
            win.blit(obj.rect, (obj.pos['x'], obj.pos['y']))
...
def randomTimes(s, e, num):
    ...
    for i in range(0, num):
        random_times.append(time_seconds + random.random())
...
def main():
    ...
    wheel_1 = creatWheel(7, 167) # Utilizo la funció objList, per automatitzar la creació de tots els objecctes de la calasse Simbol, que formaran el meu tambor
    wheel_2 = creatWheel(7, 333)
    wheel_3 = creatWheel(7, 500)


    WHEELS = [wheel_1, wheel_2, wheel_3] # Creao una llista amb
